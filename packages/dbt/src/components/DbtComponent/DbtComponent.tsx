...
function getManifest(): { manifest: Manifest, manifest_loading: boolean, manifest_error: Error } {
  ...
  let doc_bucket_path = "";
  if (entity.metadata.annotations?.["dbtdoc-bucket-path"] != null) {
    doc_bucket_path = entity.metadata.annotations?.["dbtdoc-bucket-path"].replace(/^\/|\/$/g, "");;
  }
  ...
  const { value, loading, error } = useAsync(async (): Promise<Manifest> => {
    ...
    if (doc_bucket_path) {
      params.append('bucketPath', doc_bucket_path);
